...
\begin{figure}
    ...
    \includegraphics[width=0.6\textwidth]{figures/windturbine_planner.pdf}
...
\label{alg:wind_turbine_planner}

\KwData{Mesh elements with normals and centroid locations}

\BlankLine

\textbf{A) Create the graph (clustering)}\;

$groups \gets$ group elements by normals and centroid location\;

$groups \gets$ sort elements in each group by z-value\;

\BlankLine

\textbf{B) Solve the TSP via brute force (ordering)}\;
...
\While{there are more permutations of the tour $T$}{
    ...
    \ForEach{$group$ in $groups$}{
        Append one end node in $group$ to $T$\;
        
        Append the other end node in $group$ to $T$\;
    }
    ...
    \If{distance($T$) $<$ $shortest\_distance$}{
        ...
        $shortest\_distance \gets$ distance($T$)\;
    }
}

\BlankLine

\textbf{C) Find intermediate points (interpolation)}\;

$intermediate\_points \gets$ line fitting and interpolation\;
...
\begin{figure}
    ...
    \includegraphics[width=0.6\textwidth]{figures/vtmpc_overview_1.pdf}
...
\begin{figure}[]
\centering
\includegraphics[width=.55\textwidth]{figures/distance_real.pdf}
...
\begin{figure}[]
\centering
\includegraphics[width=.48\textwidth]{figures/cm.pdf}
