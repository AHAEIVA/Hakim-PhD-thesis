...
This chapter provides a brief overview on the foundation for the design and implementation of data-driven MPC for marine robots. We begin with first-principles modeling of marine robots using a Fossen's framework \cite{fossen}, explore data-driven approaches to address modeling limitations, and conclude with MPC fundamentals, including solution methods and practical implementation details.

\section{Modelling Marine Robots}
\label{sec:modelling}
Accurate dynamic models are fundamental to MPC performance. This section develops the 6-DOF mathematical model for underwater vehicles using first principles, then examines data-driven approaches to address modeling limitations.
\subsection{Frames of Reference}
Underwater vehicle motion is typically described using two coordinate systems: an earth-fixed (global) reference frame denoted by $\mathcal{F}_E$ and a body-fixed reference frame attached to the vehicle and is denoted by $\mathcal{F}_b$.

For marine applications, the north-east-down (NED) convention system is commonly used. Its origin is often set at a convenient location (e.g., deployment point), with axes defined as follows: the $x$-axis points toward true North, the $y$-axis points toward East, and the $z$-axis points downward, perpendicular to the Earth's surface. The body-fixed reference frame, denoted $\mathcal{F}_B$, moves with the vehicle. Its origin is usually placed at the vehicle's center of gravity. Following the society of naval architects and marine engineers (SNAME) convention, the axes are defined such that the $x$-axis points forward along the longitudinal axis, the $y$-axis points starboard (right), and the $z$-axis points downward.

\begin{figure}[h!] % Changed [b!] to [h!] for better placement flexibility, adjust as needed
    \centering
    \includegraphics[width=0.5\textwidth]{Phd_thesis/chapters/1.Introduction/figures/brov4 1.pdf} % Ensure path is correct
    \caption{
        Depiction of the coordinate frames and thruster configuration of the \textit{BlueROV2} underwater vehicle. 
        The body-fixed frame, $\mathcal{F}_B$, is attached to the vehicle's center of gravity and defines the vehicle-relative axes. 
        The global frame, $\mathcal{F}_E$, represents an inertial reference frame. 
        The thruster configuration is shown, with vertical thrusters highlighted in green and horizontal thrusters in blue.
    }
    \label{fig:coordinate_frame_brov}
\end{figure}

%The vehicle's state involves position, orientation, linear velocities, and angular velocities. The position and orientation in the global frame $\mathcal{F}_E$ are represented by the vector $\boldsymbol{\eta} = [x, y, z, \phi, \theta, \psi]^T$, where $\phi$, $\theta$, and $\psi$ are the Euler angles (roll, pitch, yaw). The linear and angular velocities in the body frame $\mathcal{F}_B$ (see Fig.~\ref{fig:coordinate_frame_brov}) are represented by the vector $\boldsymbol{\nu} = [u, v, w, p, q, r]^T$. 

\subsection{Newton-Euler Equations of Motion}
The dynamics of an underwater vehicle, treated as a rigid body moving in a fluid, can be derived using the Newton-Euler equations. These account for hydrodynamic effects like added mass and damping, as well as gravitational and buoyancy forces. The equations of motion can then be written as follows:
\begin{align}
...
\begin{equation}
    ...
    \end{bmatrix}
\end{equation}
where $\mathbf{R}(\phi,\theta,\psi)$ is the rotation matrix from the body-fixed frame $\mathcal{F}_B$ to the global frame $\mathcal{F}_E$ using the ZYX Euler angle convention:
\begin{equation}
...
The added mass matrix $\mathbf{M}_A$ accounts for the inertia contribution of the fluid accelerated by the vehicle motion and can be expressed as:
\begin{equation}
    \mathbf{M}_{A} = -
    \begin{bmatrix}
    ...
    \end{bmatrix}
\end{equation}
where each coefficient $X_{\dot{u}}, Y_{\dot{v}}, \ldots, N_{\dot{r}}$ affects the hydrodynamic force or moment generated in the corresponding degree of freedom.
...
Various function approximators can be used for $\hat{f}$~\cite{brunton2025machine}. Neural networks offer great flexibility in capturing complex nonlinearities but often require large datasets and lack interpretability. For more interpretable models, sparse identification of nonlinear dynamics (SINDy)\cite{fasel2021sindy} identifies a sparse set of governing equations from a library of candidate functions. Operator-theoretic methods such as dynamic mode decomposition (DMD)\cite{schmid2010dynamic} and Koopman operator analysis~\cite{mezic2005spectral} provide linear representations of nonlinear systems by lifting dynamics into higher-dimensional spaces. Hankel matrix methods have also been applied in control contexts~\cite{willems2005note}, notably in data-driven MPC frameworks~\cite{berberich2020data}. Another approach is Gaussian process regression, a non-parametric method that provides uncertainty quantification; its hyperparameters are typically optimized by maximizing the marginal log-likelihood~\cite{rasmussen_gaussian_2008}. Among these methods, physics-informed neural networks (PINNs) incorporate first-principles knowledge to enforce physics-consistent training~\cite{raissi2019physics}, bridging data-driven and mechanistic modeling as highlighted in Figure~\ref{fig:nonlinear_modeling_classification}.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        arrow/.style={->, thick},
        dashed_arrow/.style={->, thick, dashed, TrajectoryBlue} % updated color here
    ]

    % Top level
    \node[box] (top) {Non-linear Models};

    % Second level
    \node[box, below left=of top] (data) {Data-Driven};
    \node[box, below right=of top, yshift=0.4cm, xshift=-1.0cm] (first) {First Principles};

    % Third level
    \node[box, below left=of data, yshift=0.3cm] (param) {Parametric};
    \node[box, below right=2.6cm and -1.7cm of data] (reg) {Operator-Theoretic Methods};
    \node[box, below right=of data] (nonparam) {Non-parametric};

    % Fourth level
    \node[box, below left=of param, yshift=0.7cm, xshift=-0.3cm] (nn) {Deep Learning Models};
    \node[block, below=0.5cm of nonparam] (gp) {Gaussian Process Regression};

    % Fifth level
    \node[box, below left=of nn] (baseline) {Neural Networks};
    \node[block, below right=of nn, xshift=-1.5cm, yshift=0.4cm] (pinn) {Physics-Informed Neural Networks};
    \node[box, below=0.45cm of param] (sindy) {SINDy};
    \node[box, below=1.5cm of reg, xshift=-2.4cm] (koopman) {Koopman};
    \node[box, below=1.5cm of reg] (dmd) {DMD};
    \node[box, below=1.5cm of reg, xshift=2.4cm] (subspace) {Subspace Identification \\ (Hankel)};

    % Arrows - solid black
    \draw[arrow] (top) -- (data);
    \draw[arrow] (top) -- (first);
    \draw[arrow] (data) -- (param);
    \draw[arrow] (data) -- (nonparam);
    \draw[arrow] (param) -- (nn);
    \draw[arrow] (param) -- (reg);
    \draw[arrow] (param) -- (sindy);
    \draw[arrow] (nonparam) -- (gp);
    \draw[arrow] (nn) -- (baseline);
    \draw[arrow] (nn) -- (pinn);
    \draw[arrow] (reg) -- (dmd);
    \draw[arrow] (reg) -- (koopman);
    \draw[arrow] (reg) -- (subspace);

    % Dashed arrow in TrajectoryBlue
    \draw[dashed_arrow] (first) -- (pinn);

    \end{tikzpicture}
    \caption{
    Classification of nonlinear modeling techniques. Data-driven approaches split into parametric models (neural networks, SINDy, DMD, Koopman, subspace identification) and non-parametric models (Gaussian Process regression). PINNs integrate first principles (dashed arrow) for physics-consistent training. PINNs and Gaussian process regression, highlighted in blue, are the main methods investigated.
    }
    \label{fig:nonlinear_modeling_classification}
\end{figure}
